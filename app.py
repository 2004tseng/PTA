...
def save_data():
    """將當前 session state 中的數據保存到本地 JSON 文件。"""
    data = {
        # 將 date 物件轉為 ISO 格式字串以便儲存
        'periods': [{'start': p['start'].isoformat(), 'end': p['end'].isoformat()} for p in st.session_state.periods],
...
def get_contextual_tip(stage):
    ...
    base_tip = tips.get(clean_stage, "請保持良好生活習慣，並確保輸入最新紀錄以獲得最準確的推算。")

    # 6. 特殊提醒：當今日狀態為月經期時，額外加入專屬文字
    if "月經期" in clean_stage:
        # 使用 Markdown 語法換行和加粗，使其更醒目
        base_tip += "\n\n**⚠️ 專屬提醒：** 扁曾俊瑋時請不要打臉！"
...
FUN_CONTENTS = [
    ...
    "有一隻北極熊太無聊，一直拔牠的毛，等到全部拔完之後,牠說:「好冷」",
    "為什麼小紅帽沒有胸部? 因為她的奶奶被大野狼吃掉了",
    "為什麼花木蘭沒有胸部? 因為木蘭無長兄",
    "狼愛上羊屬於什麼戀? 食物鏈",
    "有一天小美對小明說:「你能為我而死嗎?」＼n 小明默默地把手伸進耳多...",
    "有一天有個帥哥走在路上 ＼n 一個阿嬤突然上前搭訕說:「帥哥，你超會搭耶」＼n 然後帥哥就冒煙了...",
    "拔罐師傅最容易犯什麼罪? 妳拔我罐醉",
    "馬的耳多叫馬耳，馬的尾巴叫馬尾，那馬的眼睛呢? ＼n 馬鳩",
    "鄉下人為什麼都不喜歡下水游泳? 因為鄉間河太急",
    "Good、Better、Best都想上廁所，應該讓誰先去? ＼n Better，因為它是比較級(急)",
    "小蚯蚓起床後問媽媽:「爸爸去哪裡了?」＼n 蚯蚓媽媽說:「你爸爸可能去釣魚了」",
    "舉重選手失敗時會說什麼? 珍重再見(真重，再見)",
    "為什麼男生洗澡要關門? 因為怕老鷹飛出去 ＼n 為什麼女生洗澡要關門? 因為怕小鳥飛進來"
]
